...
sections below specify entry conditions and protocol changes. **EIP TBD** marks
...
The authoritative schedule is grouped into the **lifecycle track**, the
**duty track**, and the underlying **substrate**, using the transition model
defined above. Lifecycle work prepares and retires validator-facing state;
duty work flips atomically at Phase 2a; transport is independent and can move
first. Committee votes require succinct aggregation, while sole-signer
messages require only the base scheme and, for a stateful scheme, an explicit
...
not alter the authoritative schedule. Lifecycle services already map directly
to lifecycle rows, so this second view focuses on cross-cutting duty services.
Data availability is shown only to make its deliberate exclusion visible.
...
This inventory is the detailed cryptographic-surface audit. Each entry records
its purpose, current mechanism, load-bearing BLS property, migration owner,
and end state. It is descriptive rather than schedular: where it conflicts
with the phase matrix, the phase matrix wins.
...
The active stake that is post-quantum-ready is readable from the beacon state. The threshold for scheduling the
...
Phase 2 executes the cutover described above. Phase 2a switches all validator
duties to post-quantum authentication; Phase 2b closes legacy validator and
builder entry; Phase 2c removes the remaining BLS-authorized transition paths
and retires unswitched stake.

Phases 2a and 2b may share a fork. Phase 2c must follow 2b because its
force-exit bound is measured after legacy entry has closed and any permitted
late-registration window has run.
...
Post-quantum readiness saturation (open decision 1); under the opt-in aggregation
...
The unresolved choices are listed here once in full. This index shows what each
decision controls and the earliest phase it blocks.
...
The document separates destination, migration, and completeness. The target
architecture describes the protocol after transitional machinery is gone; the
phase matrix is authoritative for scheduling; the property, service,
lifecycle, and boundary views audit that schedule for omissions. Lifecycle
mechanisms change gradually, duty mechanisms flip atomically at Phase 2a, and
the independent transport substrate can precede both.
