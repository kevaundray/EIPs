...
A validator is **switched** when it holds a registered post-quantum key and an
active registered RANDAO commitment. The **fold** is the Phase 2a replacement
...
cross-cut services, needs this second view. Data availability appears only
here: its KZG commitments rely on BLS12-381 pairings but are not signatures,
so the signature-centric lists above cannot carry it, and it remains the
separate track the Abstract records.
...
### Usage index: pre-quantum, migration, post-quantum

The phase sections below schedule the transition in time order. The three
sections here index the same content by usage, under matching subheadings,
segmented the way the matrix is: stake lifecycle, then duties, then the
substrate. Pre-quantum entries state what each mechanism is for, how it
works, and what it relies on; Migration entries state the path through the
phases and what is still open; Post-quantum entries state the end state.
...
Migration carries a cross-cutting segment, history and sync, distributed
validators, and networking, which exists only while the transition runs.
Like the coverage table, these sections add nothing to the schedule.
...
- **Mechanism:**
  - `DepositData` is signed under the validator's BLS key
  - This is sent to the Deposit contract with `amount`
  - If surface level checks pass (like pubkey length), then a log is emitted
...
On the last point, imagine that no signature was needed to register a validator with a particular
...
- **Path:** the Phase 1 path records post-quantum deposits dormant; Phase 2a
  activates them while BLS deposits continue; Phase 2b stops BLS-form
  deposits creating validators; Phase 2c removes BLS deposit signature
  verification.
- **Open:** decision 2 picks the candidate, decision 3 the cutover rule.

#### Validator exits

- **Path:** the CL-triggered operation is removed at Phase 2a, leaving the
  EL-triggered path as the only way out; the credential-retirement draft
  first clears `0x00` validators, so every remaining validator holds
...
- **Path:** assigned draft 8365 exits `0x00` validators in Phase 1;
  `BLSToExecutionChange` stays open for balance recovery until Phase 2c
  removes it.
- **Open:** decision 7 rules the remaining balances.

#### Builder deposits and exits

- **Path:** post-quantum registration is recorded dormant in Phase 1; the
  BLS deposit predeploy stops being read at Phase 2b and top-ups move to
  the post-quantum request type; unrotated builders are exited at Phase 2c.
- **Open:** no decision; the builder-key EIP (TBD) specifies it.

#### Validator naming

- **Path:** Phase 1 fixes the encoding; the Phase 2a fold replaces switched
  keys in `Validator.pubkey` and retains a lookup for the pre-fold pubkey;
  `pq_pubkeys` is removed at Phase 2c.
- **Open:** decision 4 picks the `Validator.pubkey` model.

**Duties**

#### Attestations

- **Path:** signing moves to the post-quantum scheme at Phase 2a;
  aggregation becomes proof-based under the vote-aggregation EIP.
- **Open:** decision 6 picks the producer, in Phase 1.

#### Aggregator selection

- **Path:** replaced ahead of the switch, in Phase 1: assigned draft 8384 on
  the sync side, the attestation side per decision 6.
- **Open:** decision 6; only its hash-tree option registers a Phase 1
  credential.

#### Block proposals

- **Path:** flips at Phase 2a with the verification EIP.
- **Open:** nothing.

#### RANDAO

- **Path:** the [EIP-8321](./eip-8321.md) chain is registered in Phase 1,
  dormant, and goes live at Phase 2a; an active commitment is half of the
  definition of switched.
- **Open:** nothing; the required change to EIP-8321's activation timing is
  in Reconciliation.

#### Sync committee and light clients

- **Path:** if the committee is kept, the sync aggregate goes post-quantum
  at Phase 2a alongside a light-client EIP; if removed,
  [EIP-8390](./eip-8390.md) supersedes both and the Phase 1 placement of
  assigned draft 8384.
- **Open:** decision 11 comes first.

#### Proposer preferences

- **Path:** flips at Phase 2a with the gossip-authentication EIP, which
  budgets its one-time indices.
- **Open:** nothing.

#### Builder bids and payload envelopes

- **Path:** BLS bids end at Phase 2a: from the fork only bids signed under
  a post-quantum builder key are valid, and a builder that has not rotated
  cannot bid until it does.
- **Open:** nothing; the builder-key EIP (TBD) specifies the signing.

#### Payload attestations

- **Path:** the committee votes flip at Phase 2a with the vote-aggregation
  EIP.
- **Open:** nothing.

#### Inclusion lists

- **Path:** flips at Phase 2a with the gossip-authentication EIP.
- **Open:** nothing.

#### Slashing

- **Path:** the post-quantum evidence format arrives at Phase 2a; pre-switch
  BLS evidence closes no later than Phase 2c.
- **Open:** decision 10 sets the pre-switch evidence window.

#### Gossip authentication

- **Path:** flips at Phase 2a with the gossip-authentication EIP, which sets
  the one-time-index budget per message type.
- **Open:** nothing.
...
- **Path:** Phase 1, before everything else: it depends on nothing and
  nothing depends on it.
- **Open:** nothing; the transport EIP (TBD) specifies it.
...
Only post-quantum deposits create validators, carrying the key and the
RANDAO commitment bound to withdrawal credentials. Under the two
contract-deploying candidates the original contract serves only top-ups,
resolved through the retained lookup; under the reuse candidate it is
itself the post-quantum path, with its BLS-form deposits inert beyond
top-ups.

#### Validator exits

Only EL-triggered exits remain: exit authority is the withdrawal address
through [EIP-7002](./eip-7002.md), and its safety is the execution
account's, under the decision 13 track.

#### Withdrawal credential changes

Gone, with the rest of the `0x00` machinery; every validator holds
execution credentials, and leftover retired balances follow open
decision 7's rule.

#### Builder deposits and exits

Post-quantum request types for registration, top-up, rotation, and exit;
the BLS deposit predeploy is no longer read.

#### Validator naming

`Validator.pubkey` holds the post-quantum encoding under the decision 4
model; the pre-fold BLS pubkey lookup remains for immutable staking
contracts; `pq_pubkeys` is gone.

**Duties**

#### Attestations

Signed under the post-quantum scheme; committee votes reach the block as
succinct proofs; the producer is whatever open decision 6 chose.

#### Aggregator selection

No selection proof exists. Aggregators are an opt-in role, a public prefix
of the committee, or a hash-tree draw, per open decision 6.

#### Block proposals

One post-quantum signature per proposal, one one-time index spent.

#### RANDAO

The proposer reveals the next preimage of its registered hash chain;
commitments ride in deposits.

#### Sync committee and light clients

Either light clients verify a succinct proof over the committee's keys, or
there is no sync committee and public infrastructure publishes finality
proofs.

#### Proposer preferences

Post-quantum gossip-signed under the validator key, drawing from its
one-time-index budget.

#### Builder bids and payload envelopes

Signed under the registered post-quantum builder key, verified in the
state transition.

#### Payload attestations

Proof-aggregated like beacon attestations.

#### Inclusion lists

Post-quantum gossip-signed, still never aggregated.

#### Slashing

Evidence cites post-quantum votes in the Phase 2a format; pre-switch BLS
evidence is closed.

#### Gossip authentication

Every signed gossip message spends from its key's one-time-index budget
under the gossip-authentication EIP's allocation.

**Substrate**

#### Transport

Post-quantum or hybrid key agreement and a post-quantum peer identity type.
